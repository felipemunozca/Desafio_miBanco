 ...
 * > npm i pg-cursor
 * 
 * Crear una nueva BD llamada Banco
 * CREATE DATABASE Banco;
 * 
 * Crear una nueva tabla llamada transacciones en la BD
 ...
 * 
 * Crear una nueva tabla llamada cuentas en la BD
 * CREATE TABLE cuentas (
        id INT, 
        saldo DECIMAL CHECK (saldo >= 0) 
    );
 * 
 * Crear el registro de la primera transaccion en la BD
 * INSERT INTO cuentas values (1, 20000);
 * 
 */

/* Utilizo un destructoring para llamar a la clase Pool */
const { Pool } = require('pg');
/* Llamo a la Clase Cursor, por eso la primera letra es con mayuscula. */
...
const depositar = (descripcion, fecha, monto, cuenta) => {
    pool.connect(async (error_conexion, client, release) => {
        if (error_conexion){
            ...
            try {
                /* Ya que el metodo query es asincrono, si no lo esperamos con await, pasa de largo. */
                /* con BEGIN se indica que va a iniciar una transaccion. */
                ...
                /* con COMMIT se guarda la ejecucion de las consultas en la BD. */
                await client.query("COMMIT");

            } catch (error) {
                /* con ROLLBACK deshago cualquier cambio que pudo sufrir la BD y vuelve a su estado anterior antes del BEGIN. */
                await client.query("ROLLBACK");

                /* Se crea un objeto para definir diferentes tipos de errores y luego imprimirlos en una tabla por consola. */
                /* code -> imprime el codigo de error para buscar el significado en la documentacion oficial. */
                /* https://www.postgresql.org/docs/current/errcodes-appendix.html */
                /* message -> imprime un mensaje con informacion util para entender porque se produjo el error. */
                /* detail -> muestra un detalle del error. En algunos casos la respues pueda ser "undefined". */
                /* table -> que tabla de la base de datos fue afectada. En algunos casos la respues pueda ser "undefined". */
